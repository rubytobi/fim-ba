...
public class BHKW implements Device{
	...
	/**
	 *  Fahrplan in Minuten Rythmus, den der Consumer gerade aushandelt
	 *  mit Fuellstand des Reservoirs (0) und Stromerzeugung (1)
	 */
	private double[][] scheduleMinutes;
	
	/** 
	 * Fahrplaene in Minuten Rythmus, die schon ausgehandelt sind 
	...
	private void chargeChangedSchedule(double[][] changes) {
		...
		for (int i=0; i<numSlots; i++) {
			System.out.println("Änderung Gesamt:     Speicher: " +changes[0][i]+
					", Erzeugung: " +changes[1][i]);
			changesPerMinute[0][i] = Math.round(100.00 * changes[0][i]/15) /100.00;
			changesPerMinute[1][i] = Math.round(100.00 * changes[1][i]/15) /100.00; 
			System.out.println("Änderung pro Minute: Speicher: " +changesPerMinute[0][i]+
					", Erzeugung: " +changesPerMinute[1][i]);
			...
			System.out.println("Zu Erreichen: Speicher: " +valuesToAchieve[0][i]+
					", Erzeugung: " +valuesToAchieve[1][i]);
		}
				
		double achievedPower = 0;
		boolean repeat = false;
		double repeatPowerToChange = 0
				, repeatLevelToChange = 0, 
		...
		for (int n=1; n<numSlots+1; n++) {
			...
			for (int j=slotAnfang; j<slotEnde; j++) {
				// Berechne das neue Level des Wärmespeichers und die neue Stromerzeugung,
				...
				if (levelPlanned == 0 || levelPlanned == sizeHeatReservoir 
						|| powerPlanned == 0 || powerPlanned == maxLoad) {
					System.out.println("Grenzwerte sind schon vor Änderung erreicht gewesen.");
					// Übernehme für das geplante Level das Level von der vorherigen Minute
					...
					// Prüfe, ob der neu berechnete Wert die Grenezen über- oder unterschreitet
					// Wenn ja, passe ihn an und lege die Beschränkung für die Erzeugung fest
					if (newLevel < 0) {
					...
					// Passe Power so an, dass die von newLevel kommenden Restriktionen
					// eingehalten werden
					...
					// Prüfe, ob Power die festgesetzten Grenzen unter- oder überschreitet
				...
				System.out.println((j+1)+". Minute neu: Speicher: " +planned[0][j]+
						", Erzeugung: " +planned[1][j]);
				...
				if (j == slotEnde-1) {
					...
					double deviationPower = Math.round(Math.abs(valuesToAchieve[1][n-1] - achievedPower) * 100.00) /100.00;
					double deviationLevel = Math.round(Math.abs(valuesToAchieve[0][n-1] - newLevel) * 100.00) /100.00;
					...
					if (! (deviationPower == 0 || deviationLevel == 0)){
						...
						// Verkleinere die Höhe des angefragten Ausgleichs pro Minute
