...
public class Washer implements Device {
	 ...
	 * @param programs	Map aller Programme mit dem jeweiligen Namen und dem Verbrauch pro Minute für das gesamte Programm
	 */
	 ...
	 * @param schedule	Minuetlicher Fahrplan, fuer welchen Lasprofil erstellt werden soll
	 ...
	 * @param start		Start des Programms
	...
	private void chargeNewSchedule() {
		...
		for (int i = 0; i < currentMinuteSchedule; i++) {
			...
			System.out.println("Minute " +i+ " Programm: " +currentProgram+ " Verbrauch: " 
					+scheduleMinutes[i]);
		}
		
		// Wenn das Programm der letzten Stunde noch  nicht beendet ist, beende zuerst
		// dieses Programm
		System.out.println("--------Beende vorhergehendes Programm: " +(currentProgram != null)+ ", " +currentMinute);
	 ...
	 * @param values					Verbrauchswerte pro Minute des Programms
	 * @param currentMinuteSchedule		Minute des Fahrplans, in der das Programm starten soll
	 * @param minuteProgram				Minute des Programms
	...
	private int addProgram(double[] values, int currentMinuteSchedule, int minuteProgram) {
		...
		while (currentMinuteSchedule < end && currentMinuteSchedule < 15*numSlots
				&& minuteProgram < values.length) {
	...
	/**
	 * Fuegt alle Programme, die in der aktuellen Stunde starten sollen, der Queue hinzu
	 */
